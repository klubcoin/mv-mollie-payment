...
public class WorkflowMetric extends Script {
    ...
    private static String serializeOrderKey(MoOrder order) {
        return String.format("status=\"%s\",user_group=\"%s\",user=\"%s\"}",
            order.getStatus(), order.getGroup(), order.getAssignedTo());
    }
    ...
    public void execute(Map<String, Object> parameters) throws BusinessException {
        ...
        orders.stream()
              .collect(Collectors.groupingBy(WorkflowMetric::serializeOrderKey, Collectors.counting()))
              .forEach((id, count) -> {
                  result += "workflow_status_total{process=\"order\"," + id + " " + count + " " + time + "\n";
              });
    }
}
